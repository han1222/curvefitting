...
p_gain=0.2#0.05#0.18#0.06#0.15#0.06
h_gain =0.3#0.2#0.15
f_gain=0.3#0.4
dt=0.025#0.025
global prev_delta
...
def error(msg):
    ...
    cte=fx.subs({x: 9.6}) # check this today
    ...
    heading_error_term_list.append(h_gain * heading_error_term * steering_ratio * 180 / math.pi)
    ...
    if ((((delta-prev_delta)* steering_ratio * 180) / math.pi)/dt > steering_vel_constraint) :
        ...
        #low pass filter
    low_pass_gain = 0.2 #  The closer to 0, the more filtering is performed and the speed is slower. The closer to 1, the less filtering is performed and the faster the speed.
...
if __name__== '__main__':
    ...
    rospy.Subscriber("/simulation/bodyOdom",Odometry,get_odom_cb)
    rospy.spin() 
